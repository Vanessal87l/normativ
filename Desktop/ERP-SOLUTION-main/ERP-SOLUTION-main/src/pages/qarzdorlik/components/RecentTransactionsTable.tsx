...
export default function RecentTransactionsTable({ title = "So‘nggi tranzaksiyalar", rows }: Props) {
  return (
    <div className="rounded-2xl glass border border-slate-200 overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200">
        <div>
          <div className="text-sm font-extrabold text-white">{title}</div>
          <div className="mt-1 text-xs text-white">Oxirgi kirim/chiqim</div>
        </div>

        <div className="flex items-center gap-2">
          <button className="rounded-xl border border-slate-200 glass px-3 py-2 text-xs font-bold text-white hover:bg-slate-50">
            Barchasi
          </button>
        </div>
      </div>
          ...
          <tbody className="divide-y divide-slate-200 text-white">
            {rows.map((r) => (
              <tr key={String(r.id)} className="hover:bg-slate-50">
                <td className="px-4 py-3 text-white">{r.dateTime}</td>
                ...
                <td className="px-4 py-3 font-semibold text-white">{r.paymentName}</td>
